...
(function(d) {
    ...
    function z(a, b) {
        var c = new i.Api(a),
            e = b ? b : c.init().buttons || i.defaults.buttons;
                ...
                var c = function(e) {
                    for (var g = 0, f = e.length; g < f; g++) {
                        var h = e[g].conf,
                            k = e[g].node;
                        if (h.key)
                            if (h.key === a) b._buttonsHandled = !0, d(k).click();
                            else if (d.isPlainObject(h.key) && h.key.key === a && (!h.key.shiftKey || b.shiftKey))
                            if (!h.key.altKey || b.altKey)
                                if (!h.key.ctrlKey ||
                                    b.ctrlKey)
                                    if (!h.key.metaKey || b.metaKey) b._buttonsHandled = !0, d(k).click();
    ...
    i.Api.register("buttons().containers()", function() {
        ...
        this.iterator(!0, "table", function(c) {
            if (c._buttons)
                for (var c = p.instanceSelector(b, c._buttons), d = 0, g = c.length; d < g; d++) a = a.add(c[d].container())
        });
        return a
    });
